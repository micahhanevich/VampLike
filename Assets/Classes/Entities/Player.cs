...
public class Player : Entity
{
    ...
    public CardControls CardControls;
    ...
    void Start()
    {
        ...
        StatusEffects.Add(new StatusApplication(new Stun(), "test", duration: 180));
    }
    ...
    protected override void RenderStatus(StatusApplication status)
    {
        Debug.Log("Rendering Status");
        if (status.New)
        {
            // render status effect
            status.Icon = Instantiate(status.Status.Icon, StatusBar.transform);

            // run Status's OnGet method
            status.Status.OnGet(this);
            status.New = false;
        }
        else if (status.Duration >= 0)
        {
            // activate tick-based status effects
            status.Status.OnTick(this);
            status.Duration -= 1;
        }
        else
        {
            // remove status effect and apply OnEnd effects
            status.Status.OnEnd(this);
            Destroy(status.Icon);
    ...
    override protected void OnAttack(Vector2 direction)
    {
        ...
        MoveCooldownTimer = (MoveCooldown * MoveCooldownMultiplier) * MoveCooldownTotalMultiplier;
        if (AttackProjectile && !AttackLocked) { MultiAttack(direction); }
    }
    ...
    void LevelUp()
    {
        ...
        MaxXP = (int)((float)MaxXP * 1.75f);
        CardControls.SetupLevelUpMenu();
